...
- 优先提交官方页面、代码仓库、论文原文、作者或社区原帖，而非搜索结果、转载或聚合页。
...
PR 中请列出原始链接、核验日期、收录理由和实际运行的验证命令。若页面需要登录、日期无法确定、互动数字可能变化或只能找到二手来源，请明确说明，不要隐藏不确定性。一个 PR 可以收录多条相关资源，但不要混入无关格式化或代码改动。
